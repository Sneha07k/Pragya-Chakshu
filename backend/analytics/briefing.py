...
def generate_case_briefing(case_id: str) -> Dict[str, Any]:
    ...
    forum_personas = [p for p in persona_rows if "forum" in p.get("platform", "").lower() or p.get("raw_uid")]
    market_personas = [p for p in persona_rows if "market" in p.get("platform", "").lower() or p.get("raw_vid")]
    ...
    active_evidence = [e for e in evidence_rows if e.get("challenge_status") == "ACTIVE"]
    challenged_evidence = [e for e in evidence_rows if e.get("challenge_status") == "CHALLENGED"]
    ...
    for (src, tgt), ev_list in pair_evidence_map.items():
        ...
        score = sum(e.get("confidence_weight", 0.0) for e in supp) - sum(e.get("confidence_weight", 0.0) for e in conf)
        ...
        if score >= 40.0 or any("PGP" in str(r) or "handle" in str(r).lower() for r in reasons):
    ...
    try:
        ...
        for cl in clusters[:5]:
            coordination_findings.append({
                ...
                "summary": cl.get("description", "Rapid reply cascade observed across shared threads."),
            })
    except Exception:
        pass
    ...
    if infra_clusters:
        ...
        server_ip = first_cl.get("server_ip") or first_cl.get("server", {}).get("ip", "185.220.101.78")
        asn_name = first_cl.get("asn") or first_cl.get("server", {}).get("asn", "FlokiNET Offshore Datacenter")
        ...
        infra_summary_text = (
            f"No malicious co-hosting infrastructure or synchronized posting cliques have triggered threshold alerts at this time."
        )
    ...
    top_personas = []
    for p in sorted(persona_rows, key=lambda x: (x.get("raw_uid") or 0) + (x.get("raw_vid") or 0), reverse=True)[:6]:
