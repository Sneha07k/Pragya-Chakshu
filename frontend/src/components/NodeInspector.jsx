...
export default function NodeInspector({ node, caseId }) {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-3 p-4">
        ...
        <p className="text-xs text-center text-slate-400">
          Select any graph node to inspect forensic attributes and analytical
          profiles.
        </p>
      </div>
    );
  ...
  const displayEntries = Object.entries(node).filter(
    ([key]) => !["id", "label"].includes(key),
  );
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          {node.provenance && (
            <span
              className={`text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-wider font-semibold ${getProvenanceBadge(node.provenance)}`}
            >
              {node.provenance}
            </span>
          )}
        ...
        <div className="space-y-2">
          {displayEntries.map(([key, value]) => {
            ...
            return (
              <div
                key={key}
                className="bg-slate-900/90 p-2.5 rounded border border-slate-800 text-xs"
              >
                <div className="text-[10px] text-slate-500 mb-0.5 font-medium uppercase">
                  {key}
                </div>
      ...
      {activeTab === "stylometry" && (
        <div className="space-y-3 text-xs">
          {loadingAnalytics ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-900 p-2 rounded border border-slate-800">
                  ...
                  <div className="text-sm font-bold text-slate-200 font-mono mt-0.5">
                    {stylometry.avg_sentence_len} words
                  </div>
                  <div className="text-[9px] text-slate-500 mt-0.5">
                    Var: {stylometry.sentence_len_var}
                  </div>
                </div>
                <div className="bg-slate-900 p-2 rounded border border-slate-800">
                  <div className="text-[10px] text-slate-500">
                  ...
                  <div className="text-sm font-bold text-emerald-400 font-mono mt-0.5 uppercase">
                    {stylometry.language}
                  </div>
                  <div className="text-[9px] text-slate-500 mt-0.5">
                    {stylometry.total_words} words analyzed
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-1.5 font-mono text-[11px]">
                  {Object.entries(stylometry.punctuation_vector || {}).map(
                    ([p, v]) => (
                      <div
                        key={p}
                        className="bg-slate-900 p-1.5 rounded border border-slate-800 text-center"
                      >
            ...
            <div className="text-slate-500 text-center py-4">
              No text samples available for this persona.
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === "behavioral" && (
        <div className="space-y-3 text-xs">
          {loadingAnalytics ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-slate-900 p-2 rounded border border-slate-800">
                  ...
                  <div className="text-sm font-bold text-slate-200 font-mono mt-0.5">
                    {behavioral.mean_interval_hours} hrs
                  </div>
                </div>
                <div className="bg-slate-900 p-2 rounded border border-slate-800">
                  <div className="text-[10px] text-slate-500">
                    Activity Bursts (&le;1h)
                  </div>
                  <div className="text-sm font-bold text-amber-400 font-mono mt-0.5">
                    {behavioral.burst_count}
                  </div>
                </div>
                <div className="bg-slate-900 p-2 rounded border border-slate-800 col-span-2">
                  <div className="text-[10px] text-slate-500">
                    Active Observation Span
                  </div>
                  <div className="text-xs text-slate-200 font-mono mt-0.5">
                    {behavioral.active_span_days} days (
                    {behavioral.total_events} events)
                  </div>
                </div>
              </div>
            </>
            ...
            <div className="text-slate-500 text-center py-4">
              No timestamped events recorded for this persona.
            </div>
          )}
        </div>
      )}
